...
def test_ht_not_saved_to_disk(ht):
    a = ht.save(
        filename=os.path.join(HERE, "test_output", "test_hough_transform_npy"),
        fileformat="npy",
        to_disk=False,
...
def test_extending_ht(ht):
    ht2 = HoughTransform()
    ht2.set_constraints(min_slope=1, max_slope=4, min_intercept=7000.0, max_intercept=9000.0)
...
@pytest.mark.xfail()
def test_extending_ht_expect_fail(ht):
    ht.add_hough_points(np.ones(100))


def test_loading_ht_into_calibrator():
    c = Calibrator(np.arange(10))
    c.load_hough_transform(os.path.join(HERE, "test_output", "test_hough_transform_npy"))
    c.save_hough_transform(os.path.join(HERE, "test_output", "test_hough_transform_npy_2"))
