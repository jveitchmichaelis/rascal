...
spectrum2D = fits.open(
    '/Users/marcolam/git/GMOSfieldflattening/example/gaia2018cnz/output/N20181115S0215_flattened.fits'
)[0].data
# Collapse into 1D spectrum between row 150 and 160
spectrum = np.median(spectrum2D[145:165], axis=0)[::-1]
...
# Identify the peaks
peaks, _ = find_peaks(spectrum, prominence=500, distance=10, threshold=None)
...
def run_calibration(polydeg, peaks):
    # Run the wavelength calibration
    best_p, rms, residual, peak_utilisation = c.fit(sample_size=5,
                                                    top_n=20,
                                                    max_tries=10000)
...
for i in range(n):
    print(i)
    best_p, _, peak_utilisation[i], rms[i], waves[i] = run_calibration(
        4, peaks)
