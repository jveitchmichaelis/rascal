...
class Sampler(ABC):
    ...
    def __init__(self, x, y, sample_size, n_samples=-1):
        ...
        Args:
            x (list(float)): input x
            y (list(float)): input y
            sample_size (_type_): size of sample to return
...
class UniformRandomSampler(Sampler):
    ...
    def _setup(self):
        ...
        significantly if there are a lot of 
        """
        ...
        if self.n_samples == -1:
            self.logger.warning("Returning all samples as number of samples was not provided.")
...
class WeightedRandomSampler(UniformRandomSampler):
    ...
    def _truncate_samples(self, max_bins=10):
        ...
        prob = 1.0 / hist[np.digitize(self.unique_x, bin_edges, right=True) - 1]
        ...
        try:
            self.samples = np.random.choice(self.samples, size=self.n_samples, replace=False, p=weight)
        except ValueError:
            self.logger.error(f"Unable to draw {self.n_samples} unique samples from population.")
